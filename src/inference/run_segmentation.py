...
def display_images_with_segmentation(input_folder, display_time=20):
    ...
    # Modify this for presentation: Only include files that start with a number
    image_files = [img for img in image_files if img.split('.')[0].isdigit()]
    image_files.sort(key=lambda x: int(x.split('.')[0]))

    visualizer = SegmentationVisualizer(
        model_path_1="D://Code//Self-Driving//saved_models//lane_segmentation_model//best_lane_detection.pt",
        model_path_2="D://Code//Self-Driving//saved_models//object_detection_model//yolo11s-seg.pt",
        conf_threshold=0.5
    )
...
display_images_with_segmentation(input_folder, display_time=100)  # Images are displayed for 100 milliseconds (0.1 seconds)
